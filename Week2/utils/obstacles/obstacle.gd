...
func animate():
	...
	sprite_tween.bind_node(self)
	shadow_tween.bind_node(self)
...
func _on_hurt_box_area_entered(area):
	...
	if "Sloower" in name:
		var opposite = -((area.global_position - global_position).normalized() * 64) + global_position
		hitParticles.look_at((opposite))
